...
const ProductsPage: React.FC = () => {
	...
	const { data: allProducts, isPending: isLoading } = useQuery({
		queryKey: ["products-worker"],
		queryFn: getProducts,
	});
	const fabrics = useMemo(() => {
		return allProducts?.filter((p) => p.type === "fabric");
	}, [allProducts]);
	...
	const { mutate } = useMutation({
		...
		mutationFn: updateProductsAvailability,
		onSuccess: () => {
	...
	const updateAvailability = async (id: string, isAvailable: boolean) => {
		try {
			await mutate({ id, available: isAvailable });
		} catch (err) {
							...
							{displayedItems?.map((item) => (
								...
								>
									...
									<div className="flex items-center space-x-4">
										<div className="flex items-center space-x-2">
											<span
												className={`inline-block w-2 h-2 rounded-full ${item.available ? "bg-green-500" : "bg-red-500"}`}
											/>
