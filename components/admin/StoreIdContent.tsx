...
export default function StoreDetailsPage({ id }: { id: string }) {
	...
	const { data: store, isPending: isLoadingStore } = useQuery({
		queryKey: ["store", id],
		queryFn: () => getStoreById(id),
	});
	const { data: products, isPending: isLoadingProduct } = useQuery({
		queryKey: ["products", id],
		queryFn: () => getStoreProducts(id),
	});
	const { data: orders, isPending: isLoadingOrder } = useQuery({
		queryKey: ["orders", id],
		queryFn: () => getStoreOrders(id),
	});
	const { data: requests, isPending: isLoadingRequest } = useQuery({
		queryKey: ["requests", id],
		queryFn: () => getStoreRequests(id),
	});

	const isLoading =
		isLoadingStore || isLoadingProduct || isLoadingOrder || isLoadingRequest;
				...
				<main className="flex-1 p-6 space-y-6">
					...
					<div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
						...
						<Tabs
							value={currentTab}
							onValueChange={(v) =>
								setCurrentTab(v as "products" | "orders" | "requests")
							}
